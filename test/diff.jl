...
import TurbulenceFlux: init_wavelet_parameters, GaussConv, WaveletConv
using ForwardDiff
...
WaveC = WaveletConv((work_dim, 1), (wave_dim, size(wave_params, 2)))

@test begin
    d = ForwardDiff.jacobian(
        wave_params -> WaveC.kernel_func(WaveC, wave_params),
        wave_params,
    )
