...
@testset "FFT-based Convolutional Kernel" begin
    ...
    @testset "Skipping Kernel Load" begin
        ...
        cconv = CConv(Float64, work_dim, kernel_dim; phase = 0)
        y = cconv(x, kernel)
        y_bis = cconv(x, randn(work_dim); load_kernel = false) # Dont update kernel
        kernel_bis = randn(work_dim)
        y_bis_2 = cconv(x, kernel_bis) # Update kernel
        @test isapprox(y, kernel)
...
import TurbulenceFlux: averaging_kernel, TimeParams, average, _default_phase_kernel
@testset "Averaging Kernels" begin
    ...
    @testset "Average func" for kernel_type in [:gaussian, :rect], kernel_dim in [16, 15]
        ...
        x = circshift(vcat(1, zeros(work_dim - 1)), _default_phase_kernel(kernel_dim))
        # No padding
        tp = TimeParams(kernel_dim, kernel_type, kernel_params)
        x_avg = average(x, tp)
...
@testset "Cross-Scalogram test" for work_dim in [8192, 8191]
    ...
    x1, x2, x3 = L
    out = cross_scalogram(x1, x1, dp, gmw, avg_kernel)
    @test isapprox(sum(out), 1) # The flux is one between x1 and x1
    out = cross_scalogram(x1, x2, dp, gmw, avg_kernel)
    @test isapprox(sum(out), 0, atol = 1e-6) # The flux is zero between x1 and x2
    out = cross_scalogram(x1, x3, dp, gmw, avg_kernel)
    ...
    @test isapprox(sum(out[(1, 2)]), 0, atol = 1e-6)
    @test isapprox(sum(out[(1, 3)]), 1)
    @test isapprox(sum(out[(2, 3)]), 1)
    @test isapprox(out[(1, 2)], out[(2, 1)])
end
