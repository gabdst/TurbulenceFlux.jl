...
# Assuming u,v,w the three wind speed components (m/s), P (Pa) and T (K) pressure and temperature signals and C (umol/mol) the CO2 concentration

work_dim=length(u) # size of signals
fs=20 # Sampling frequency FREQUENCY
...
σ=10*60*fs/kernel_dim # 10min averaging length, Gaussian reaches 0.6 at 10min, 0.6^2 at 20min.
...
# Prepare density and mean wind amplitude (for simplicity here using the previous time-decomposition but not necessarily as long as Δt is the same)
...
# Turbulence extraction based on the laplacian of τ_w
